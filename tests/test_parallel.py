...
from nox import _merge, _options, _parallel, tasks
...
def _config(**kwargs: object) -> _options.NoxConfig:
...
def test_parallel_env_invalid_errors_cleanly(monkeypatch: pytest.MonkeyPatch) -> None:
    # A bad NOX_PARALLEL is reported through parser.error, like any other
...
def test_parallel_env_valid_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert _options.options.parse_args([]).parallel == 3


def test_invalid_noxfile_parallel_errors() -> None:
    # nox.options.parallel bypasses argparse; the field converter rejects a
    # bad value right at the assignment, not with a traceback deep in the run.
    with pytest.raises(ValueError, match="parallel"):
        _options.options.namespace(parallel="bogus")


def test_zero_noxfile_parallel_errors() -> None:
    # 0 is invalid like on the command line, not a silent "sequential".
    with pytest.raises(ValueError, match="parallel"):
        _options.options.namespace(parallel=0)
...
def test_parallel_noxfile_settable() -> None:
    assert hasattr(_options.NoxfileOptions(), "parallel")
...
def test_allow_parallel_noxfile_and_cli_merge() -> None:
    ...
    for argv, noxfile_value, expected in [
        ...
        config = _options.options.parse_args(argv)
        noxfile_args = _options.NoxfileOptions()
        noxfile_args.allow_parallel = noxfile_value
        _merge.merge_noxfile_options(config, noxfile_args)
        assert config.allow_parallel is expected, (argv, noxfile_value)
